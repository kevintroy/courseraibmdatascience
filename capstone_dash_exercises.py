...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider', min = 0, max = 20000, step = 1000, marks = {0: '0', 20000: '20000'}, value = [min_payload, max_payload]),
...
def get_pie_chart(entered_site): 
    if entered_site == 'ALL':
        data = spacex_df.groupby('Launch Site', as_index=False).sum('class')
        fig = px.pie(data, 
                    values = 'class', 
                    names = 'Launch Site', 
        ...
        data = data.groupby('outcome', as_index=False).count()
        fig = px.pie(data, 
                    values = 'class', 
                    names = 'outcome',
...
def get_scatterplot(entered_site, payload_range): 
    ...
    fig = px.scatter(data, 
                    ...
                    title = 'Landing Outcomes by Booster Version Category and Payload Mass')
